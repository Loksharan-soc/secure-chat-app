...
if (fread(qCand, 1, qLen, f) != qLen) {
    fprintf(stderr, "Error reading qCand from file\n");
    // Handle error (e.g., return, exit, etc.)
}
			BYTES2Z(q,qCand,qLen);
		} while (!ISPRIME(q));
		/* now try to get p */
if (fread(rCand, 1, rLen, f) != rLen) {
    fprintf(stderr, "Error reading rCand from file\n");
    // Handle error (e.g., return, exit, etc.)
}
...
if (fread(tCand, 1, tLen, f) != tLen) {
    fprintf(stderr, "Error reading tCand from file\n");
    // Handle error (e.g., return, exit, etc.)
}
...
if (fread(buf, 1, buflen, f) != buflen) {
    fprintf(stderr, "Error reading buffer from file\n");
    // Handle error (e.g., return, exit, etc.)
}
	fclose(f);
	NEWZ(a);
